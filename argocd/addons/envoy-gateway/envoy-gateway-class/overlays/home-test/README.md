# envoy-gateway-class - home-test cluster

This folder contains cluster-specific Kustomize patches for deploying **envoy-gateway-class** to the **home-test** cluster.
...
- `envoy-gateway-class-appset.yaml` - ArgoCD ApplicationSet for deployment
...
```bash
kustomize build overlays/home-test/
```
...
```bash
kubectl apply -k overlays/home-test/
```
...
```bash
kubectl diff -k overlays/home-test/
```
